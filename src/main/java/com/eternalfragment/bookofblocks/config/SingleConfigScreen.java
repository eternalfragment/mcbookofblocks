...
public class SingleConfigScreen {
    ...
    public static void buildScreen(String itemName) {
        ...
        //Researchable Option
        general.addEntry(entryBuilder.startStringDropdownMenu(Text.translatable("bob.config.menu.researchable"), Integer.toString((Integer) ob[1]))
                ...
                .build());
        //Research Amount
        general.addEntry(entryBuilder.startIntField(Text.translatable("bob.config.menu.resAmt"), (Integer) ob[2])
                .setDefaultValue((Integer) ob[2])
                .setSaveConsumer(result->addReqAmt.set(result))
                .build());
        //Give Amount
        general.addEntry(entryBuilder.startIntSlider(Text.translatable("bob.config.menu.giveAmt"), (Integer) ob[3],1,64)
                .setDefaultValue((Integer) ob[3])
                .setSaveConsumer(result->addGiveAmt.set(result))
                .build());
        //Scoreboard Objective
        general.addEntry(entryBuilder.startStrField(Text.translatable("bob.config.menu.scbName"),(String) ob[4])
                .setDefaultValue((String) ob[4])
                .setSaveConsumer(result->addScbObj.set(result))
                .build());
        //Scoreboard Amount
        general.addEntry(entryBuilder.startIntField(Text.translatable("bob.config.menu.scbGoal"), (Integer) ob[5])
        ...
        builder.setSavingRunnable(()->{
            ...
            ClientPlayNetworking.send(Bookofblocks.get_single_config_packet, data);//send the config data set here to the server for parsing/saving.
        });
